BEGIN TRANSACTION;
CREATE TABLE IF NOT EXISTS `lectie` (
	...
	`partea9`	REAL,
	`intrebare`	INTEGER,
	`rezultate`	INTEGER DEFAULT 0,
	`partinr`	INTEGER DEFAULT 0
);
INSERT INTO `lectie` VALUES (1,1,'Geografie ușor','Hidrografia Europei<<-->>Europa deține o retea hidrografică bogată cu dimensiuni impresionate.
 În continuare vor fi prezentate fluviile si raurile din top 5:
...
•Rinul este una dintre căile fluviale cele mai circulate din Europa si are o lungime de 1230 km.
 ','Munții<<-->>Există cel puțin 109 munți pe Pământ cu altitudini mai mari de 7.200 metri deasupra nivelului mării. Marea majoritate a acestor munți se află la marginea plăcii Indiene și Eurosiatice.
...
•Precipitațiile sunt abundente, sau relativ abundente tot anul: aproximativ 1000-2000 mm/an
 •Bat vânturile de vest și alizeele
  ...
  • Apele Dunării, care se varsă în Marea Neagră, formează cea mai mare și mai bine conservată deltă din Europa. Delta Dunării găzduiește peste 300 de specii de păsări, precum și 45 de specii de pești de apă dulce în numeroasele sale lacuri și mlaștini.
    ','America Centrală <<-->>America Centrală este o regiune centrală a celor două Americi. Nedefinită exact, regiunea geografică este considerată a fi porțiunea sudică a continentului nord-american.
...
',NULL,NULL, NULL,'Dunărea traversează 4 capitale?<->A<<-->>Rinul este cel mai lung fluviu din Europa?<->A<<-->> Cel mai înalt munte de pe glob este Everest?<->A<<-->>Permafrostul este solul ce se găsește in regiunea polară?<->A<<-->> Alizeele sunt vânturile tipice din climatul oceanic ?<->A<<-->>Este România o țară Carpato-Danubiano-Pontică?<->A<<-->>  Americii Centrale se invecinează cu Caraibe<->F<<-->>În București poti vizita Colosseum?<->F<<-->> Este Brazilia  una dintre țările traversate de linia Ecuatorului?<->A<<-->>Globul pământesc este compus din 7 tectonice principale?<->A',0,7);

INSERT INTO `lectie` VALUES (3,1,'Geografie Mediu','Enclave<<-->>Enclavele au fost un fenomen relativ răspândit în geografia politică a Europei medievale și chiar moderne.
...
',NULL,NULL, NULL, NULL,'Care este cea mai mică capitală și stat enclavă din lume<->Vatican<->Berlin<->Ciudad de Mexico<->Seoul<<-->>Care este cel mai întins lac cu apă dulce din Europa?<->Baikal<->Rybinsk<->Saimaa<->Estona<<-->>Cu ce ocean se învecinează Europa în partea vestică?<->Atlantic<->Pacific<->Indian<->Artic<<-->>Unde este situată cascada Niagara?<->la granita dintre America si Canada<->Mexic<->Australia<->Brazilia<<-->>Unde se află Marea Bariera de Corali?<->Australia<->Japonia<->Indonezia<->Jamaica<<-->>Pe teritoriul cărei țări se află vulcanul Fuji?<->Japonia<->Hawaii<->Stromboli<->USA<<-->> Care stat european este renumit pentru calitatea ciocolatei?<->Belgia<->Franta<->Finlanda<->Spania<<-->>Care este cea mai mare stațiune balneo-climaterică din Europa?<->Baden Baden<->Grindavik<->Vals<->Bath<<-->> În ce regiune din America de Sud se află Machu Picchu<->Peru<->Columbia<->Brazilia<->Bolivia<<-->>Cel mai mare aeroport din Europa este localizat în ?<->Charles de Gaulle<->Heathrow<->Amsterdam<->Frankfurt',0,6);

INSERT INTO `lectie` VALUES (5,3,'Geografie Special','Provocare<<-->> Ești pregătit să faci față ultimei provocări?
 Celelalte teste au fost doar încălzirea.
',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'Care este cel mai întins continent?<->Asia<->Antartica<->America de Nord<->Africa<<-->>Este Canada cea mai apropiată țară  de Polul Nord?<->T<<-->>Ce este un arhipelag?<->O adunare sau grupare de mai multe insule <->O sectiune de pamant aproape fiind inconjurata de ape<->O insula mica, de mica inaltime, pe un recif de corali<->O intindere de uscat care se prelungeste spre un corp de apa <<-->>În Islanda, puteți traversa pe doua placi tectonice. Care dintre ele, mai exact?<->EuroAsiatica si Nord Americana<->EuroAsiatica si Sud Americana<->EuroAsiatica si Antartica<->EuroAsiatica si Indo-Australiana<<-->>Care este cel mai uscat deșert din lume în afara regiunilor polare??<->Atacama<->Kalahari<->Gobi<->Sahara<<-->>Kazakhstan este țara care produce cel mai mult uraniu de pe mapamond?<->T<<-->> În Africa de Sud se află Cascada Victoria<->T<<-->>În Insula Paștelui se găsește piatra Moai?<->T<<-->> Prin ce țară traversează Tropicul Capricornului?<->Australia<->China<->Mexic<->Bolivia<<-->>Fort Knox se găsește în statul american Kentucky? ?<->T',0,1);


INSERT INTO `lectie` VALUES (2,2,'Anatomie usor','Oasele<<-->>Osul reprezintă elementul de bază al scheletului vertebratelor, caracterizat prin structura lui dură, solidă și rezistentă.
...
',NULL,'Este pielea cel mai mare organ al corpului uman?<->A<<-->>Tibia este cel mai lung os al corpului?<->F<<-->>Inima este compusă din 4 camere?<->A<<-->>Artera aortă este cea mai mare aortă din corp?<->A<<-->>Un om ajuns la maturitate are 42 de dinți?<->F<<-->>Scheletul unui om ajuns la maturitate are între 206 și 210  de oase?<->A<<-->>Hemofilia este o boală specifică sistemului nervos?<->F<<-->>Keratina se regăsește în unghii și firul de păr?<->A<<-->>Epiglota se regăsește în torace?<->F<<-->>Lobul parietal este responsabil cu controlul motrocității?<->F',0,9);


INSERT INTO `lectie` VALUES (4,2,'Anatomie mediu','Ochiul<<-->>Ochiul este un organ a cărui principală funcție este cea de a detecta lumina. Se compune dintr-un sistem sensibil la schimbările de lumină, capabil să le transforme în impulsuri nervoase.
...
• Alveole pulmonare (în plămâni)de unde oxigenul trece în sânge, apoi în celule.
 ',NULL,NULL,NULL,NULL,NULL,'Cum se numește incapacitatea de a percepe culorile roșu și verde?<->daltonism<->cataracta<->strabism<->miopie<<-->>Ce denumire mai poartă celula nervoasa?<->neuron<->nucleu<->sinapsa<->celula eucariota<<-->>Din câte falange sunt formate degetele de la mână(fără cel mare)<->3<->4<->2<->5<<-->>Care este rolul rinichilor?<->filtrarea toxinelor<->eliminare urina<->absorbtie nutrienti<->eliminare depuneri<<-->>Unde este localizată Trompeta lui Eustache?<->ureche<->tibie<->torace<->coloana vertebrala<<-->>Rolul globulelor albe este?<->de a proteja organismul<->optimizarea vederii nocturne<->mentinerea echilibrului<->produc enzime<<-->>Din câte regiuni este formată coloana vertebrală?<->5<->4<->3<->2<<-->>Unde se regăsește plasma?<->sange<->limfa<->maduva<->os<<-->>Câte  perechi coaste are un om normal<->12<->6<->10<->8<<-->>Unde se găsesc sacii alveolari?<->plamani<->inima<->ficat<->pancreas',0,5);


INSERT INTO `lectie` VALUES (6,3,'Anatomie special','Provocare<<-->> Ești pregătit să faci față ultimei provocări?
 Celelalte teste au fost doar încălzirea.
',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL, 'Artera subclaviană dreaptă este localizată sub artera carotidă?<->F <<-->> Care arteră este principala sursă de sânge pentru braț?<->artera brahială<->artera ulnară<->artera radială<->artera profundă <<-->> Artera femurală este a doua arteră ca mărime din corp?<->T <<-->> Un exemplu al articulației pivotante este?<->articulatiile radioulnare<->metacarpofalangiene<->articulațiile gleznei<->articulațiile șoldului <<-->> Glezna este formată din 6 oase?<->F <<-->>Kazakhstan este țara care produce cel mai mult uraniu de pe mapamond?<->T<<-->> Urechea internă are în componență?<->endolimfă<->perilimfă<->limfă<->mezolimfă <<-->> Toți nervii spinali sunt sub o formă mixtă?<->T <<-->> Prin ce țară traversează Venele hepatice există în număr de 3?<->A <<-->> Cărui element nu se aplică teoria celulei?<->virus<->alge<->bacterii<->fungi',0,1);
CREATE TABLE IF NOT EXISTS `curs` (
	`_id`	INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE,
	`titlu`	TEXT
);
INSERT INTO `curs` VALUES (1,'Geografie');
INSERT INTO `curs` VALUES (2,'Anatomie');
INSERT INTO `curs` VALUES (3,'Spicy');
